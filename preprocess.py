...
# 对时间戳间隔进行统一，并对缺失值部分补0，同时返回一个array指示某一步是否为缺失值
def complete_timestamp(timestamp, arrays=None):
    ...
    intervals = np.unique(np.diff(timestamp_sorted)) # calculate the interval o[n] = a[n+1]-a[n]
    interval = np.min(intervals) # select the minimum interval as Interval
    if interval == 0:
    ...
    dst_index = np.asarray((timestamp_sorted - timestamp_sorted[0]) // interval,
                           dtype=np.int)
    ret_missing[dst_index] = 0 # 对应位置的索引为0，表示此处的timestamp未缺失
...
def standardize_kpi(values, mean=None, std=None, excludes=None):
    ...
    return np.clip((values - mean) / std, -10, 10), mean, std
