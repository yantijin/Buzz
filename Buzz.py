...
class Buzz():
    ...
    def build_q_net(self, x, outdimension=64):
        ...
        with tf.variable_scope('build_q_net', reuse=tf.AUTO_REUSE):
            ...
            with tf.name_scope('ZMEAN'):
                z_mean = fc(lz_x, self._z_dim, 'z_mean', activation_func=None, batch_normalization=False)
    ...
    def genModel(self, z, in_dimension=64, output_num=128): 
        with tf.variable_scope('genModel', reuse=tf.AUTO_REUSE): 
            ...
            lx_z = deconv(lx_z, in_dimension*2, 'deconv1', activation_func=tf.nn.leaky_relu, batch_normalization=True)
            lx_z = deconv(lx_z, in_dimension*2, 'deconv2', stride_size=2, activation_func=tf.nn.leaky_relu, batch_normalization=True)
            lx_z = deconv(lx_z, in_dimension*4, 'deconv3', activation_func=tf.nn.leaky_relu, batch_normalization=True)
            lx_z = deconv(lx_z, 1, 'deconv4', stride_size=2, activation_func=tf.nn.leaky_relu, batch_normalization=False)

            
            resShape = lx_z.get_shape().as_list()
            lx_z = tf.reshape(lx_z, [batch_size, resShape[1] * resShape[2] * resShape[3]])

            lx_z = fc(lx_z, output_num, 'deconvOut', activation_func=None, batch_normalization=False)
    ...
    def autoEncoder(self, x, outDimension=64):
        with tf.name_scope('Variational_Net'):
            mu, sigma = self.build_q_net(x, outDimension)
        ...
        with tf.name_scope('GenerationNet'):
            Xhat = self.genModel(z, outDimension)
    ...
    def discNet(self, x, outdimension=32):
        with tf.variable_scope('discNet', reuse=tf.AUTO_REUSE):
            ...
            dis = fc(lz_x, 1, 'dis', batch_normalization=False, activation_func=None)
...
#        with tf.variable_scope('DisGenZ'):
        with tf.name_scope('DisGenZ'):
            dis2 = self.discNet(Xhat)
        
        with tf.name_scope('wgan_loss'):
            wgan_loss = tf.reduce_mean(dis1-dis2) # shape (1)

        # get gradient penalty
        batch_size = x.get_shape().as_list()[0]
        kxi = tf.random_uniform(shape=[batch_size, window_size])
        ...
        with tf.variable_scope('omegaLoss', reuse=tf.AUTO_REUSE):
            ...
            logZ = log(factorial(window_size) / factorial(int(window_size/2)) * 2 * pow(pi, window_size/2)) - window_size * tf.math.log(la)
            Wloss = -la * tf.math.abs(wgan_loss) + negKL - logZ

        return Xhat, negKL, wgan_loss, gradientPenalty, vloss, Wloss, logZ, la
...
#    labels = np.zeros_like(values, dtype=np.int32)
    ...
    # standardize the training and testing data and clip the data by [-10, 10]
    ...
    Xhat, negKL, wgan_loss, gradientPenalty, vloss, Wloss, logZ, la = model.getLoss(x, eta)
...
#    writer = tf.summary.FileWriter('./logs', sess.graph)
    ...
    while s0 > 0:
        ...
        for critic in range(stopNum//b0):
            ...
            if critic%4==3:
                _, GenLoss, WL, GP, _KL, LZ, barX, LAM = sess.run([Gen_loss, -Wloss, wgan_loss, gradientPenalty, negKL, logZ, Xhat, la], feed_dict={x:inputs})
                print('epoch: {} GenLoss: {} s0: {}  b0: {}, wgan_loss: {}, GP: {}, -KL:{}, LZ:{}, LAM:{}'.format(epoch, GenLoss, s0, b0, WL, GP, _KL, LZ, LAM))
            else:
                _, DisLoss, WL, GP, barX, LAM = sess.run([Dis_loss, -vloss, wgan_loss, gradientPenalty,Xhat, la], feed_dict={x:inputs})
                print('epoch: {} DisLoss: {} s0: {}  b0: {}, wgan_loss: {}, GP: {}, LAM:{}'.format(epoch, DisLoss, s0, b0, WL, GP, LAM))
                #print(barX)
        epoch += 1
